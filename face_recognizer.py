...
class Face:
    ...
    def load_train_file_by_name(self, customer_name):
        trained_storage = path.join(self.storage, 'trained')
        return path.join(trained_storage, customer_name)

    def load_unknown_file_by_name(self, customer_name):
        unknown_storage = path.join(self.storage, 'unknown')
        return path.join(unknown_storage, customer_name)

    def load_all(self):
        with sql.connect("database.db") as con:
            cur = con.cursor()
            results = cur.execute('SELECT faces.id, faces.user_id, faces.filename, faces.created FROM faces')
            for row in results:
                user_id = row[1]
                filename = row[2]
